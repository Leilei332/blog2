...
首先是操作界面。iSH在键盘上增加了 `TAB` `CTRL` `ESC` 和方向键这四个键，不过这个方向键的操作方式很奇怪，四个方向键是并在一起的，在这个按钮上拖动才能操作方向键。这个设计可能是iSH对于终端不支持点击的补偿（原因见[*此处*]{.spurious-link target="*外层"}）。
c
...
虽然iSH目前在iOS的体验非常差，而且[*小问题*]{.spurious-link target="*一些小问题"}一大堆。但是由于
